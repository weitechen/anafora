...
PropertyFrame.prototype.removeAObjFromProperty = function(pIdx, pListIdx) {
		var propType = currentAProject.selectedAObj.type.propertyTypeList[pIdx];
		...
		if(removeAObj instanceof Entity)
			currentAProject.removeEntityPosit(removeAObj, currentAProject.selectedAObj);
		else
			currentAProject.removeRelationPosit(removeAObj, currentAProject.selectedAObj);

		currentAProject.updateOverlap(removeAObj);
...
PropertyFrame.prototype.modifyCurrentProperty = function(value) {
	if(this.currentSelectedPropertyIdx != undefined) {
		...
		var propType = currentAProject.selectedAObj.type.propertyTypeList[this.currentSelectedPropertyIdx];
		var prop = currentAProject.selectedAObj.propertyList[this.currentSelectedPropertyIdx];
		...
		if(this.isAssignRelation && value instanceof IAnaforaObj && propType.input == InputType.LIST && propType.instanceOfList.indexOf(value.type) != -1) {
			...
			if(prop == undefined)
				...
				if(prop.length == propType.maxlink) {
					var lastElement = currentAProject.selectedAObj.propertyList[this.currentSelectedPropertyIdx][prop.length-1];
					if(lastElement instanceof Entity) {
						currentAProject.removeEntityPosit(lastElement, currentAProject.selectedAObj);
						currentAProject.updateOverlap(lastElement);
					}
...
PropertyFrame.prototype.generatePropertyTableRow = function(pType, pIdx, pValue, isDiffProp, comparedIdx) {
	...
	if(this.editable) {
		...
		tRow.find(".propertyAObj").bind("click", function(evt) {
			...
			schemaDiv.jstree("selectAObjList", checkedTypeList, !(_self.isAssignRelation));
			//_self.restore();
			...
			hide: function(opt) {
				...
				if(origValue != newValue) {
					...
					if(currentAProject.selectedAObj instanceof AdjudicationEntity) {
						...
						if(currentAProject.selectedAObj.updateProperty(pTypeIdx))
