...
IAnaforaObj.prototype.destroy = function() {
	...
	for(var i=0;i<this.linkingAObjList.length;i++) {
		...
		console.log("linkingAObj: " + linkingAObj.id);
...
AdjudicationEntity.prototype.updateProperty = function(pTypeIdx) {
...
AdjudicationEntity.prototype.addCompareAObj = function(aObj1, aObj2) {
	...
	if(aObj1 != undefined && aObj2 != undefined && aObj1.type == aObj2.type) {
		...
		$.each(this.type.propertyTypeList, function(idx) { if(aObj1.propertyList[idx] !== aObj2.propertyList[idx]) _self.diffProp.push(idx);});
		...
		else if(aObj1.type == aObj2.type && Entity.sort(aObj1, aObj2) == 0 && (function (a,b) { return !(a<b || b<a); }(aObj1.propertyList,aObj2.propertyList))) {
			this.decideIdx = 0;
