...
function updatePropertyFrameProperty(pTypeIdx) {
	if(currentAProject.selectedAObj instanceof AdjudicationEntity) {
		...
		var diff = adjEntity.diffProp.indexOf(pTypeIdx) >= 0;
...
function moveAnnotation(step, adj) {
	var positIdx = Object.keys(currentAProject.positIndex);
	var startdx;
	var spanIdx = 0;
	var newEntity;
	if(positIdx.length > 0) {
		...
		if(currentAProject.selectedAObj == null && !adj) {
			...
			var currentSpanIdx = (currentAProject.selectedAObj == null ) ? spanIdx : currentAProject.positIndex[startIdx].indexOf(currentAProject.selectedAObj);
			spanIdx = currentSpanIdx;
			do {
				if((step == -1 && spanIdx == 0)||(step == 1 && spanIdx == currentAProject.positIndex[startIdx].length-1)) {
				...
				newEntity = currentAProject.positIndex[startIdx][spanIdx];
...
function showRelationFrame() {
	...
	console.log("show relation frame");
