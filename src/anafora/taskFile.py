...
class TaskFile(object):
    ...
    def __init__(self, fileName:    str):
        ...
        if fileName.count('.') <= 3:
            raise ValidationError("XML File format error ('%s'), should be '$taskName.$schemaName.$annotator.("
                                  "completed/inpgrogress).xml'" % (str(fileName)))
        ...
        if taskTerms[-2] == "completed":
            ...
            raise ValidationError("XML File format error ('%s'): please check the value of 'completed/inprogress'. "
                                  "%s" % (str(fileName), TaskFile.formatStr))
        ...
        if len(schemaTerm) == 1:
            ...
            if schemaTerm[2] != "Adjudication":
                raise ValidationError("XML File format error ('%s'): please check the value of your adjudication "
                                      "indicator. %s" % (str(fileName), TaskFile.formatStr))
            self.schemaName = schemaTerm[0]
            self.modeName = schemaTerm[1]
            self.isAdjudication = True
        else:
        ...
        self.taskName = ".".join(taskTerms[:-4])
