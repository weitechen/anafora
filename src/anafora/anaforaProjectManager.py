...
from anafora.taskFile import TaskFile
...
class AnaforaProjectManager(object):
    #rootPath = ""
    projectList = {}
    ...
    def loadCorpus(ps, projectName):
        ...
        if not AnaforaProjectManager.projectList[projectName]:
    ...
    def checkExist(ps, projectName, corpusName = "", taskName=""):
        ...
        if not os.path.isdir(os.path.join(settings.ANAFORA_PROJECT_FILE_ROOT, projectName, corpusName, taskName)):
    ...
    def searchAvailableTask(ps, projectName, corpusName, annotator, schemaName, modeName = None, maxNumOfAnnotator = 2, needPreannotation = None):
        ...
        for taskName in AnaforaProjectManager.getAllTask(ps, projectName, corpusName):
            ...
            if (numOfAnnotatorFile < maxNumOfAnnotator or (numOfAnnotatorFile > 0 and annotator == "gold") or (
                    os.path.exists(os.path.join(taskPath, ".nolimit")))) and (not needPreannotation or
                                                                              hasPreannotation) and (not hasGold) and\
                    (not hasAdjudication) and taskName not in completedTask and taskName not in inProgressTask and \
                    not os.path.exists(os.path.join(taskPath, ".noassign")):
    ...
    def searchAvailableAdjudicationTask(ps, projectName, corpusName, adjudicator, schemaName, modeName = None, maxNumOfAnnotator = 2):
        ...
        for taskName in AnaforaProjectManager.getAllTask(ps, projectName, corpusName):
            ...
            if numOfAnnotatorFile >= maxNumOfAnnotator and (not hasOtherAdjudicator or os.path.exists(
                    os.path.join(taskPath, ".nolimit"))) and taskName not in completedTask and taskName not in \
                    inProgressTask and (not hasGold):
                newTask.append(taskName)
